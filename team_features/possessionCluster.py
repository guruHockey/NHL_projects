...
df = pd.read_excel(f'{your_file_path}pos_clustering_data.xlsx')

# converts to dataframe
df = pd.DataFrame(df)

# preprocessing data for cluster evaluation
features = df[['usat_fenwick%', 'sat_corsiFor%', 'faceoffWinPct', 'giveawaysPer60', 'avgGoalsFor', 'avgGoalsAgainst']]
...
input_x = 'usat_fenwick%'
input_y = 'giveawaysPer60'

# map clusters for graphing
plt.figure(figsize=(10, 6))
plt.scatter(df[input_x], df[input_y], c=df['Cluster'], cmap='viridis', marker='o', s=100)
plt.title('K-Means Clustering of Hockey Teams')
plt.xlabel(input_x)
plt.ylabel(input_y)
plt.colorbar(label='Cluster')
for i, player in enumerate(df['team']):
    plt.annotate(player, (df[input_x][i], df[input_y][i]), textcoords="offset points", xytext=(0,10), ha='center')
plt.grid()
plt.show()

# # for output, references your above filepath 
# Template_Path = (f'{your_file_path}pos_clustering_data.xlsx')
...
#      df.to_excel(writer, sheet_name='pos_clustering_data', startcol=0, startrow=0, index=False, header=True)